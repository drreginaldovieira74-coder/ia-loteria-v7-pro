...
st.title("LOTOELITE PRO v52.1")
st.markdown("**Moldura 5x5 | Colunas | Repetidas | PDF | Fix Progresso**")
...
def analisar_ciclo(df, config):
    ...
    for idx, row in df.iterrows():
        ...
        vistas.update(nums)
        if len(vistas) >= total:
            ciclos.append({"fim": atual[-1], "duracao": len(atual), "dezenas": set(nums)})
            atual = []
            vistas = set()
...
def gerar_jogo(config, analise, modo, filtros):
    ...
    for _ in range(200):
        ...
        if modo == "ULTRA_FOCUS" and len(falt) >= total:
            ...
            qf = min(int(total*0.6) if modo=="SUPER_FOCUS" else int(total*0.4), len(falt))
            if qf>0:
                jogo.extend(random.sample(falt, qf))
...
def gerar_pdf(jogos, config):
    ...
    c.drawString(20*mm, 280*mm, "LOTOELITE PRO v52.1 - {}".format(config["nome"]))
    c.setFont("Helvetica", 9)
    y = 265
    for i, jogo in enumerate(jogos, 1):
        txt = "J{:02d}: {}".format(i, "-".join("{:02d}".format(n) for n in jogo))
        c.drawString(15*mm, y*mm, txt)
        y -= 7
        if y < 15:
            c.showPage()
            y = 280
...
tab1, tab2, tab3, tab4 = st.tabs(["Gerador", "Filtros", "Desdobramento", "Stats"])
...
with tab1:
    ...
    if st.button("GERAR", type="primary"):
        ...
        if PDF_AVAILABLE:
            ...
            st.download_button("PDF", pdf, "jogos.pdf", "application/pdf")

with tab2:
    f = {"ativo": st.checkbox("Ativar Filtros", True)}
    col1, col2 = st.columns(2)
    with col1:
        f["soma"] = {"ativo": True, "min": st.number_input("Soma min", 150, 250, 199), "max": st.number_input("Soma max", 150, 250, 210), "fases": ["MEIO","FIM"]}
        f["pares"] = {"ativo": True, "min": st.slider("Pares min", 5, 10, 7), "max": st.slider("Pares max", 5, 10, 8)}
        f["repetidas"] = {"ativo": True, "min": st.slider("Rep min", 0, 10, 5), "max": st.slider("Rep max", 0, 10, 9)}
    with col2:
        if config["nome"] == "Lotofacil":
            f["moldura"] = {"ativo": st.checkbox("Moldura", True), "min": st.slider("Mold min", 6, 14, 8), "max": st.slider("Mold max", 6, 14, 11)}
            f["colunas"] = {"ativo": st.checkbox("Colunas", True), "min": st.slider("Min colunas", 3, 5, 4)}
...
with tab3:
    ...
    if st.button("Desdobrar"):
        jogos = desdobrar(config, analise, base)
        st.success("{} jogos".format(len(jogos)))
