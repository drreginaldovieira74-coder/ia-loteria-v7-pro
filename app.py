...
st.markdown("**Plataforma Profissional de Previsão Inteligente** • Ciclo + IA + Simulador")
...
# TAB 2 - GERAR JOGOS COM FILTROS AVANÇADOS
with tab2:
    ...
    with col2:
        pares = st.slider("Números pares", 0, config["sorteadas"], config["sorteadas"]//2)
    with col3:
        consecutivos = st.slider("Máx. consecutivos", 1, 6, 3)
    ...
    if st.button("🚀 Gerar Jogos com Filtros", type="primary", use_container_width=True):
        ...
        for _ in range(qtd):
            while True:
                ...
                num_consec = max([jogo[i+1] - jogo[i] for i in range(len(jogo)-1)])
...
# TAB 4 - SIMULADOR HISTÓRICO AVANÇADO
with tab4:
        ...
        if jogos_teste.strip():
            ...
            for jogo in jogos:
                ...
                resultados.append({
                    ...
                    "Média de acertos": round(np.mean(acertos), 1),
                    "Acertos em FIM de ciclo": "Em breve"
                })
