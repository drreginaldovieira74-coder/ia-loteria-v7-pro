...
# TAB 5 - BACKTESTING (melhorado)
with tab5:
    st.subheader("📉 Backtesting Automático com IA")
    total_carregados = len(df)
    n_teste = min(200, total_carregados)
    
    if total_carregados < 100:
        st.warning(f"⚠️ Histórico pequeno ({total_carregados} concursos). Carregue o CSV completo desde o primeiro concurso para resultados mais confiáveis.")
    else:
        st.success(f"✅ Usando os últimos **{n_teste}** concursos (de {total_carregados} carregados)")
